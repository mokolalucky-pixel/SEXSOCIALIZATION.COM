...
export async function getPartnerForUser(userId) {
  const [relationship] = await getSql()`
    SELECT partner_invites.id, owner_user_id, partner_user_id,
      owner.email AS owner_email, partner.email AS partner_email,
      owner.display_name AS owner_display_name, partner.display_name AS partner_display_name
...
export async function requirePartnerForUser(userId) {
  ...
  if (!partner) throw Object.assign(new Error('Accept a partner invite before using this feature.'), { statusCode: 400 })
  return partner
}

export async function hasActiveRelationship(userId) {
  return Boolean(await getPartnerForUser(userId))
}
