...
export default function MixedCirclePage() {
  ...
  useEffect(() => {
    ...
    const checkAccess = async () => {
      ...
      setAccess(data.ageVerified && !!data.partnerId)
    }
    checkAccess()
  ...
  useEffect(() => {
    if (!access) return
    const q = query(collection(db, 'circles', 'mixed', 'posts'), orderBy('createdAt', 'desc'))
  ...
  const handlePost = async (e) => {
    ...
    setPosting(true)
    try {
      await addDoc(collection(db, 'circles', 'mixed', 'posts'), {
        text: newPost.trim(),
        authorId: user.uid,
        ...
        <div className="pt-24 px-4 flex items-center justify-center min-h-screen">
          <div className="card text-center max-w-md">
            ...
            <p className="text-white/60 text-sm">
              You need a <strong className="text-[#e91e8c]">linked partner</strong> to access the Mixed Circle.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="auth-bg min-h-screen">
      ...
      <div className="max-w-2xl mx-auto pt-24 px-4 pb-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Mixed Circle</h1>
            <p className="text-white/50 text-sm">Couples learning from each other</p>
          </div>
        </div>
        ...
        <form onSubmit={handlePost} className="card mb-6">
          <textarea
            ...
            placeholder="Share something with all couples…"
